...
// not there", and the shipped copy — under FALCONET_HOME, the tool's own
// checkout — was unreachable by any documented move. A binary has no
// FALCONET_HOME for a default to fail to resolve against: the prompt is in
// it, the default config no longer names a path, and the config key is an
// override and nothing else. The bug is impossible rather than fixed.
//
...
// .md — and whether there is one. The bash built "$FALCONET_HOME/prompts/
// $NAME.md" from the name as given, so `prompt ../README` printed falconet's
// own README; an embed.FS opens nothing but a clean relative file name, so a
