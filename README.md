...
| Check | `falconet check` after every agent pass, with the workflow owning the loop: a failing check is another pass, at most `max-attempts` times. Then the guards in `falconet commit`: path allowlist, the refusal of CI and automation configuration, content denylist, rename refusal, secret scan, the config file itself, and the checkout's own git machinery, once, terminally |
