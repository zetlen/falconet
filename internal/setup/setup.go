...
// # Why init exists
//
// Setup was eight steps of `gh`, typed by hand, and the part of adopting
...
// each step's write into one too. Steps 2, 6 and 7 need no token (#10);
// steps 4 and 5 need one (#11); step 3's App is registered by manifest
...
// the config key that names it. The descriptions are README step 5's
...
var style = map[string]struct{ Color, Description string }{
	...
	"pr":         {"5319e7", "Opened by falconet: the change and its plan need review"},
}

// Labels is step 5 as a plan: for each of the four, whether to create it,
...
// --- step 6: the config and the prompt -------------------------------------

// PromptPath is where init copies the shipped prompt, and what the config
// it writes points prompts.implement at: README step 6's "copy the file
...
// ConfigJSON is .github/falconet.json as init writes it: the one prompt
// override, indented two spaces, one trailing newline, nothing else —
// every other key has a default that transfers (README step 6), and a file
// that repeats the defaults is a file that drifts from them.
...
// workflowTemplate is README step 7's file, post-cutover (#19): the
...
// coordinate, ADR-0006 D6), the three secrets, and the two blocks whose
...
const (
	...
	LeftLabels = "step 5 — create the four labels: falconet init with FALCONET_SETUP_TOKEN set   " +
		"(or: for l in infra-request needs-info ready-for-human needs-plan-review; do gh label create \"$l\"; done)"
	LeftPrompt = "step 6 — edit the standing-facts block in " + PromptPath + ": it describes the repository falconet was extracted from " +
	...
	LeftPromptUnset = "step 6 — prompts.implement is not set, so the shipped prompt is used, and its standing facts are the origin's: " +
	...
	LeftCanary = "step 8 — file a canary issue: the smallest change the repository can carry (one DNS record, one tag), " +
