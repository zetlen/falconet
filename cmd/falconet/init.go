...
// internal/doctor so the two verbs are the same code; the sealed box is
// internal/secrets. This file is the
...
const initUsageText = `init — install falconet into this repository: README steps 2–7, each
...
the idempotent one. The labels (5), then the secrets (4, then 3), then
the local files (2, 6, 7), then one commit. A token short of a permission
...
Exit codes: 0 = everything attempted succeeded (skipped steps are not
            ...
            1 = a dirty tree, a refused write, a
...
type initFlags struct {
	repo, explicit, appID, appKey       string
	appName, appTimeout                 string
	replace, noCommit, noApp, noBrowser bool
...
func runInit(args []string) int {
	...
	// The file the caller hands in — the App's PEM — resolves against where
	// the caller stands, before the cd to the root. A relative --config does
	// NOT: it resolves from the repository root after the cd, as it does for
	// every other verb (the rule 35c98e6 set), so the config init installs
	// against is the one doctor then confirms and the pipeline's verbs read.
	if f.appKey != "" && !filepath.IsAbs(f.appKey) {
		f.appKey = filepath.Join(cwd, f.appKey)
	}
	...
	// label names, the handoff directory — reads it.
	...
	// --- preflight: the input that is a file ----------------------------------
	//
	// Validated before any write, so a malformed file costs nothing.
	var appKey []byte
	if f.appKey != "" {
	...
	// --- step 5: the labels, the first write ----------------------------------
	//
	...
	switch {
	case token == "":
		say(doctor.Line{Status: doctor.Skipped, Step: 5, Text: "the four labels (no FALCONET_SETUP_TOKEN)"})
		left.add(leftLabels, setup.LeftLabels)
	case errLabels != nil:
		say(doctor.Refused(5, "the four labels", errLabels, needsIssuesWrite))
	...
	default:
		for _, step := range setup.Labels(want, sortedKeys(existingLabels)) {
			if step.Create == nil {
				say(doctor.Line{Status: doctor.OK, Step: 5, Text: "label " + step.Name + " exists"})
				continue
			}
			...
			switch {
			case err == nil:
				say(doctor.Line{Status: doctor.Done, Step: 5, Text: "label " + step.Name + " created"})
			case errors.As(err, &e) && e.Status == http.StatusUnprocessableEntity:
				say(doctor.Line{Status: doctor.OK, Step: 5, Text: "label " + step.Name + " exists"})
			default:
				return refusedWrite(5, "could not create label "+step.Name, err, needsIssuesWrite)
			}
		}
	}

	// --- steps 4 and 3: the secrets ---------------------------------------------
	//
	...
	// Step 6: the config and the prompt. A config that exists is kept — and
	// checked as doctor checks it; what it names wrong is left for a person.
	// One written names the prompt copy.
	if !writeConfig {
		...
		for _, p := range promptsOnDisk(cfg) {
			...
			if line.Status == doctor.Missing && p.Inside && !p.Exists {
				...
				say(doctor.Line{Status: doctor.Done, Step: 6, Text: fmt.Sprintf("prompts.%s names %s, copied from the shipped prompt", p.Key, p.Path)})
				if p.Key == "implement" {
		...
		if !namesImplement {
			say(doctor.Line{Status: doctor.Note, Step: 6, Text: "prompts.implement is not set, so the shipped prompt is used, and its standing facts are the origin's"})
			left.add(leftPrompt, setup.LeftPromptUnset)
		}
	} else {
		if rc := write(".github/falconet.json", setup.ConfigJSON(), 0o644, "the prompt override (README step 6)"); rc != 0 {
			return rc
		}
		say(doctor.Line{Status: doctor.Done, Step: 6, Text: ".github/falconet.json written (prompts.implement: " + setup.PromptPath + ")"})
		if isRegularFile(setup.PromptPath) {
			say(doctor.Line{Status: doctor.OK, Step: 6, Text: "prompts.implement names " + setup.PromptPath + ", which exists"})
		} else {
			if rc := copyPrompt("implement", setup.PromptPath, write); rc != 0 {
				return rc
			}
			say(doctor.Line{Status: doctor.Done, Step: 6, Text: "prompts.implement names " + setup.PromptPath + ", copied from the shipped prompt"})
			left.add(leftPrompt, setup.LeftPrompt)
		}
	}

	// Step 7: the caller workflow, pinned to this binary's own version. One
	...
	if text, err := os.ReadFile(doctor.WorkflowPath); err == nil {
		...
		if rc := write(doctor.WorkflowPath, setup.Workflow(ref), 0o644, "the caller workflow, "+setup.Uses(ref)+" (README step 7)"); rc != 0 {
			return rc
		}
		say(doctor.Line{Status: doctor.Done, Step: 7, Text: doctor.WorkflowPath + " written (uses " + setup.Uses(ref) + ")"})
	} else {
...
func copyPrompt(key, path string, write func(string, []byte, os.FileMode, string) int) int {
	...
	return write(path, text, 0o644, "the shipped prompt, copied; its standing-facts block needs editing (README step 6)")
}
